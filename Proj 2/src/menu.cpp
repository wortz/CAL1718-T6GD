...
void searchSupermarket(){
	string toSearch;
	int choice=0;
	vector<Road *> road_aux;
	vector<int> dists_aux;
	cout << "\nType a street name to see if there is a supermarket in it:\n";
	cin.ignore();
	getline(cin, toSearch);
	if(company->searchRoad( toSearch, road_aux, dists_aux)){
		cout<<road_aux[0]->getName()<< " found!\n";
		if(searchSuperinVertexes(road_aux[choice-1]->getName()))
					cout<<"Yes. There is a supermarket in "<<road_aux[0]->getName()<<" street\n";
		else cout<<"No. There is NOT a supermarket in "<<road_aux[0]->getName()<<" street\n";
	}

	else{
		sortVectors(road_aux, dists_aux);
		cout<<"Street name not found.\nClosest results:\n\n";
		cout<<"     Street Name\t\tDistance from input\n";
		for(unsigned int i=0;i<road_aux.size();++i){
			cout<< i+1<< " - ";
			cout<<road_aux[i]->getName()<<"\t\t"<<dists_aux[i]<<"\t\t" <<endl;
		}
		cout<<"Pick an option: "<<endl;
		cin>>choice;
		if(searchSuperinVertexes(road_aux[choice-1]->getName()))
			cout<<"Yes. There is a supermarket in "<<road_aux[choice-1]->getName();
		else cout<<"No. There is NOT a supermarket in "<<road_aux[choice-1]->getName();
	}
}

bool searchSuperinVertexes(string s){
	vector <Vertex *> aux=company->getVertexes(s);
	for(auto it:aux){
		if(company->isSuper(it))
			return true;
	}
	return false;
}
...
void sortVectors(vector<Road *> & roadList, vector<int> &indexes){
	...
	for(int i=1;i<30;++i){
		for(unsigned int j=0;j<indexes.size();j++){
			if(indexes[j]==i){
				indexes_aux.push_back(indexes[j]);
				road_aux.push_back(roadList[j]);
			}
		}
	}
	roadList=road_aux;
