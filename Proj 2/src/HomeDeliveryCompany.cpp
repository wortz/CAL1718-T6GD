...
bool HomeDeliveryCompany::searchRoad(string toSearch, vector<Road *> & roadList, vector<int> &indexes){
	for(auto it:roads){
			if(it->getName()== toSearch) { //problema se o utilizador pesquisar por exemplo "Rua"
				roadList.push_back(it);
				return true;}
		}
	vector<int> distanceIndex;
	unsigned int min=20;
	for(auto it:roads){   //vê a distancia de todos os nomes a toSearch
		int dist=editDistance(toSearch,it->getName());
		distanceIndex.push_back(dist);
		if(dist<min)  //guarda o menor indice encontrado
				min=dist;
	}
	int elegible=min+4;
	for(unsigned int i=0;i<roads.size();++i){    //guarda em roadList as ruas com distancia menor que 5 em relacao a min
		if(distanceIndex[i]<=elegible){
			roadList.push_back(roads[i]);
			indexes.push_back(distanceIndex[i]);
		}
	}
	return false;
}

vector <Vertex *> HomeDeliveryCompany::getVertexes(string streetName){
	vector <Vertex *> aux;
	for (auto it: graph->getNodes()){
		for(auto iter:it->getAdjacent()){
			if(iter.getName()==streetName)
				aux.push_back(it);
		}
	}
	return aux;
...
bool HomeDeliveryCompany::isSuper(Vertex * v){
	for(int i=0;i<supermarkets.size();i++){
		if(supermarkets[i]->getNode()->getInfo()==v->getInfo())
			return true;
	}
	return false;
}
